...
class NeiCommand extends Command {
  ...
  async run({ argv }) {
    ...
    Config.question_facade.choices = Object.keys(WBNeidata);
    try {
      // facade 类名选择
      facadeAnswer = await prompt(Config.question_facade);
    } catch (err) { return; }

    result.facadeName = facadeAnswer.facadeName;
    Config.question_apiSelect.choices = WBNeidata[facadeAnswer.facadeName].map(item => ({
      name: item.path,
      value: item.id
    }));
    ...
    try {
      // facade 类目 api 选择
      const { apiList } = await prompt(Config.question_apiSelect);
  ...
  async handleFileGenerate(targetData) {
    ...
    // TODO: 文件夹是否存在检查，避免报错
    const fileStatus = {
      controllerFileStatus: fileGenerator.mkdirs(path.resolve(cwd, nei_config.controller_placement), async () => {
        await fileGenerator.verifyDirStatus(path.resolve(cwd, nei_config.controller_placement), targetData.className, 'controller')
      }),
      serviceFileStatus: fileGenerator.mkdirs(path.resolve(cwd, nei_config.service_placement), async () => {
        await fileGenerator.verifyDirStatus(path.resolve(cwd, nei_config.service_placement), targetData.className, 'service')
      }),
      apiFileStatus: fileGenerator.mkdirs(path.resolve(cwd, nei_config.api_placement), async () => {
        await fileGenerator.verifyDirStatus(path.resolve(cwd, nei_config.api_placement), targetData.className, 'api')
      }) 
    }
    targetData.prefixName = nei_config.prefixName;
  ...
  async fetchWBNeidata(groupName, flag) {
    let cacheFacadeData = {};
    if (existsSync(path.join(__dirname, '../../data/temp/facadeList.json'))) {
      cacheFacadeData = require(`../../data/temp/facadeList.json`);
    }
    ...
    if (res && res.data && res.data.result) {
      ...
      writeFileSync(`${__dirname}/../../data/temp/facadeList.json`, JSON.stringify(tableSource), 'utf8');
      spinner.stop();
      return tableSource;
    }
    return false;
