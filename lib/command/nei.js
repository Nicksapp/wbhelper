...
class NeiCommand extends Command {
  ...
  async handleFileGenerate(targetData) {
    ...
    const fileStatus = {
      controllerFileStatus: await fileGenerator.verifyDirStatus(path.resolve(cwd, nei_config.controller_placement), targetData.className),
      serviceFileStatus: await fileGenerator.verifyDirStatus(path.resolve(cwd, nei_config.service_placement), targetData.className),
      apiFileStatus: await fileGenerator.verifyDirStatus(path.resolve(cwd, nei_config.api_placement), targetData.className)
    }
    fileTypes.map(type => {
      if (type === 'router') {
  ...
  async fetchWBNeidata(groupName, flag) {
    ...
    if (!flag && Object.keys(cacheFacadeData).length) {
      log.default('发现缓存文件,自动读取缓存内容,强制读取最新 api 数据请执行 ever nei -f');
