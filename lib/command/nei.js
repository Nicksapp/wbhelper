...
class NeiCommand extends Command {
  ...
  async handleFileGenerate(targetData) {
    ...
    fileTypes.forEach(type => {
      fileGenerator.isTargetFolderExist(path.join(cwd, nei_config[`${type}_placement`]));
    });

    // scan to get file status
    const fileStatus = {
    ...
    fileTypes.map(type => {
      ...
      // generate mock files
      fileGenerator.generateMockFile({
        pathname: nei_config.mock_placement,
        facadeName: targetData.facadeName,
        pathList: targetData.data.map(i => i.path)
      })
    });
