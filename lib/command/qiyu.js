...
class QiyuCommand extends Command {
  ...
  async run({ argv }) {
    ...
    const { staffId, accountNum, interval, fromTitle } = sessionConfig;
    let accountArr = [];
    ...
    if (continued && typeof continued === 'number') {
      setInterval(async () => {
        ...
        await axios.get(`http://localhost:7007/qiyu?accountArr=${accountId}&interval=${interval}&isProduction=${!!isProduction}`);
