...
const mockFileTPL = require('../lib/TEMPLATE/mockFileTpl.hbs');
...
fileGenerator.handleRouterGenerate = (targetData, fileName = 'router') => {
  ...
  try {
    if (!existsSync(filePath)) { // 不存在 router.ts 直接写入
      ...
      let targetFile = readFileSync(filePath, 'utf8');
      const reg = new RegExp(`controller.${nei_config.prefixName ? nei_config.prefixName + '.': ''}(\\w+).(\\w+)`, 'gm');
...
// mock 文件生成，已生成则忽略
fileGenerator.generateMockFile = ({pathname, facadeName, pathList}) => {
  if (pathList && pathList.length) {
    pathList.forEach(methodName => {
      const dirPath = path.join(process.cwd(), pathname, facadeName);
      const filePath = path.join(dirPath, methodName);

      fileGenerator.mkdirs(dirPath, () => {
        try {
          if (!existsSync(`${filePath}.json`)) {
            writeFileSync(`${filePath}.json`, mockFileTPL(), 'utf8');

            log.success(`${filePath}.json 文件 create 成功`);
          }
        } catch (err) {
          log.error(`${filePath}.json 文件 create 失败： ${err}`)
        }
      })
    })
  }
