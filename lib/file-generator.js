...
fileGenerator.handleFileContentReplace = (targetData, fileStatus, fileName, type) => {
  ...
  const fileStatusOff = Object.keys(fileStatus).filter(item => fileStatus[item] === 'off');
...
fileGenerator.verifyDirStatus = async (pathname, className) => {
  try {
    ...
    if (existFile) {
      const controllfileData = await readFileSync(path.resolve(pathname, `${existFile}`), 'utf8');
      // 检查文件，判断是否关闭 wbNei 的控制
      return fileGenerator.verifyFileOnControl(controllfileData, {
...
fileGenerator.isTargetFolderExist = (folderPath) => {
  try {
