...
app.get('/qiyu', function (req, res) {
  // 用户组， 消息发送间隔
  const { accountArr, interval = 5000, isProduction = false } = req.query;
  res.send(req.query);
  ...
  if (accountArr) {
    ...
    arr.length && arr.forEach(async (accountId) => {
      try {
        await handlePostMessageToStaff(accountId, interval, isProduction);
...
const handlePostMessageToStaff = async (accountId, interval = 5000, isProduction) => {
